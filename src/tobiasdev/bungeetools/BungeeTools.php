...
use tobiasdev\bungeetools\commands\PingCommand;
...
class BungeeTools extends PluginBase
{
    ...
    public function onEnable()
    {
        ...
        if ((bool)self::$config->getNested("overwrite-commands.ping") === true) {
            $this->getServer()->getCommandMap()->register($this->getName(), new PingCommand(self::$config->getNested("command-config.ping.description"), self::$config->getNested("command-config.ping.usage"), self::$config->getNested("command-config.ping.noperm-message"), self::$config->getNested("command-config.ping.permission")));
